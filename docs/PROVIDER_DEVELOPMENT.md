...
The current provider implementations are examples of distinct patterns: Stripe is an API-first Payment Intent adapter; bKash uses a token grant and explicit confirmation after hosted approval; SSLCommerz creates a hosted session and requires customer/billing data. Do not flatten these into a common flow where that would lose security or lifecycle semantics.
